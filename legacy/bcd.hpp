...
extern int __padd (char *, int, int,
                    const char *, int, int,
                    const char *, int, int);
extern int __padd63 (char *, int, int,
                     const char *, int, int,
                     const char *, int, int);
extern int __psub (char *, int, int,
                    const char *, int, int,
                    const char *, int, int);
extern int __psub63 (char *, int, int,
                     const char *, int, int,
                     const char *, int, int);
extern int __pmpy  (char *, int , int,
                    const char *, int,
                    const char *, int);
extern int __pmpy63  (char *, int , int,
                      const char *, int,
                      const char *, int);
...
extern long long __dectoll(unsigned char *, int, int);     // BCD to long long
#endif
...
#define DEC63_MIN (_ConvertDecimal)\
("-999999999999999999999999999999999999999999999999999999999999999")
#define DEC63_MAX (_ConvertDecimal)\
("999999999999999999999999999999999999999999999999999999999999999")
#define DEC63_EPSILON (_ConvertDecimal)\
...
#define DEC63_PRECISION 63
  ...
  #define DFT_DEC_DIG       63
  #define DFT_DEC_PRECISION 63
#else
  #define DFT_DEC_DIG       31
  #define DFT_DEC_PRECISION 31
#endif

#define DEC_INT_DIG     10
#define DEC_INT_PREC    0

#define __MAX_BUF_SIZE    ((DFT_DEC_DIG >> 1) + 1)
...
// ************************************
// imax is the largest of the integer components
#define imax(dig1, prec1, dig2, prec2) (dig1-prec1>dig2-prec2?          \
                                        dig1-prec1:dig2-prec2)
// pmax is the largest precision
#define pmax(prec1, prec2) (prec1>prec2?prec1:prec2)
// nmax is imax + pmax (number of digits required for largest possible value)
...
// *** prec = if (nmax >= DFT_DEC_DIG) then          ***
// ***          if ((imax + 1) > DFT_DEC_DIG) then   ***
// ***            0                                  ***
// ***          else                                 ***
// ***            DFT_DEC_DIG - (imax + 1)           ***
// ***        else                                   ***
// ***          pmax                                 ***
// *** dig  = if (nmax >= DFT_DEC_DIG) then          ***
// ***          DFT_DEC_DIG                          ***
// ***        else                                   ***
// ***          nmax + 1 (1 added because neg - neg  ***
// ***                    could be 1 digit bigger)   ***
// *****************************************************
#define _ADD_RET_TYPE(dig1,prec1,dig2,prec2)                            \
          (nmax(dig1,prec1,dig2,prec2)>=DFT_DEC_DIG?                    \
           DFT_DEC_DIG:                                                 \
           nmax(dig1,prec1,dig2,prec2)+1),                              \
          (nmax(dig1,prec1,dig2,prec2)>=DFT_DEC_DIG?                    \
            ((imax(dig1,prec1,dig2,prec2) + 1)>DFT_DEC_DIG?             \
             0:                                                         \
             DFT_DEC_DIG-(imax(dig1,prec1,dig2,prec2) + 1)):            \
            (pmax(prec1, prec2)))
...
#define _MUL_RET_TYPE(dig1,prec1,dig2,prec2)                            \
              (dig1+dig2>DFT_DEC_DIG?DFT_DEC_DIG:(dig1+dig2)),          \
              (dig1+dig2>DFT_DEC_DIG?                                   \
                 (dig1+dig2-prec1-prec2>=DFT_DEC_DIG?                   \
                  0:                                                    \
                  DFT_DEC_DIG-(dig1+dig2-prec1-prec2)):                 \
                 (prec1+prec2))

// *************************************************
// *** Intermediate result of division :         ***
// *** dig = DFT_DEC_DIG                         ***
// *** prec = DFT_DEC_DIG - ((dig1 - prec1) + prec2) ***
// *************************************************
#ifdef _DECIMAL_FULL_PRECISION
#define _DIV_BUFSZ(dig1,dig2) DFT_DEC_DIG
#else
#define _DIV_BUFSZ(dig1,dig2) (((dig1 < 31) && (dig2 < 31)) ? DEC_DIG : \
                                                              DFT_DEC_DIG)
#endif
#define _DIV_RET_TYPE(dig1,prec1,dig2,prec2)                            \
               _DIV_BUFSZ(dig1,dig2),                                   \
               (((dig1-prec1)+prec2)>_DIV_BUFSZ(dig1,dig2)?             \
                0:                                                      \
                (_DIV_BUFSZ(dig1,dig2)-(dig1-prec1)-prec2))

// ************************************************************
// *** Macro used by decimal<> = decimal<> OP integer       ***
// *** where OP is + - * /                                  ***
// ************************************************************
...
// ************************************************************
// *** Macro used by decimal<> = integer OP decimal<>       ***
// *** where OP is + - * /                                  ***
// ************************************************************
...
// *** where OP is + - * /                                  ***
// ************************************************************
#define GEN_1ST_DEC_TEMPLATE_FUNC_RET_FLT(_OPERATOR,_TYPE,_RET_TYPE)    \
template<int dig, int prec>                                             \
inline _RET_TYPE                                                        \
operator _OPERATOR ( const _DecimalT<dig, prec> & val1,                 \
                     const _TYPE & val2)                                \
...
// *** where OP is + - * /                                  ***
// ************************************************************
#define GEN_2ND_DEC_TEMPLATE_FUNC_RET_FLT(_OPERATOR,_TYPE,_RET_TYPE)    \
template<int dig, int prec>                                             \
inline _RET_TYPE                                                        \
operator _OPERATOR (const _TYPE & val1,                                 \
                    const _DecimalT<dig, prec> & val2)                  \
...
public:
   ...
   explicit  inline _ConvertDecimal (const char * val)
                                             throw(_DecErrInvalidConst);
#else
   inline _ConvertDecimal (const char * val) throw(_DecErrInvalidConst);
#endif
...
inline _ConvertDecimal::_ConvertDecimal (const char * val)
      throw(_DecErrInvalidConst)
{
   ...
   char tempVal[DFT_DEC_DIG * 2 + 1];
   int retErr = 0;

   memset (tempVal, 0, DFT_DEC_DIG * 2 + 1);
   ...
   const char * decPosPtr = strstr( val, ".");
   digVal = strlen(val);
   ...
   if (decPosPtr != NULL)                       // contains '.'
   {
      ...
      if ((digVal == 0) || (digVal > DFT_DEC_DIG))
      {
        _DecErrInvalidConst DecErr;       // Invalid # of digits or precision
        throw (DecErr);
      }
#endif
      counter = 0;
      NumDig = 0;
      for (const char * ptr = val;
           ptr < decPosPtr;
           ptr++, counter++, NumDig++ )
      {
         notAllZero += (tempVal[DFT_DEC_DIG - decVal + counter] = *ptr - '0');
#if (_DEBUG || _DEBUG_DECIMAL) && !_NODEBUG_DECIMAL
         if (*ptr < '0' || *ptr > '9')
         {
      ...
      counter = 0;
      for (const char *ptr2 = decPosPtr + 1;
           *ptr2 != '\0';
           ptr2++, counter++, NumDig++)
      {
         notAllZero += (tempVal[DFT_DEC_DIG + counter] = *ptr2 - '0');
#if (_DEBUG || _DEBUG_DECIMAL) && !_NODEBUG_DECIMAL
         if (*ptr2 < '0' || *ptr2 > '9')
         {
            _DecErrInvalidConst DecErr;         // Invalid digit
...
#if (_DEBUG || _DEBUG_DECIMAL) && !_NODEBUG_DECIMAL
      if (NumDig > DFT_DEC_DIG)
      {
         _DecErrInvalidConst DecErr;          // Number of digits > DFT_DEC_DIG
         throw (DecErr);
      }
#endif
   }
...
#endif
      counter = 0;
      for (const char *ptr = val; *ptr != '\0'; ptr++, counter++)
      {
         notAllZero += (tempVal[DFT_DEC_DIG - decVal + counter] = *ptr - '0');
#if (_DEBUG || _DEBUG_DECIMAL) && !_NODEBUG_DECIMAL
         if (*ptr < '0' || *ptr > '9')
...
private:
   char buf[((dig >= 1) && (dig <= DFT_DEC_DIG)) ? (dig/2 + 1) : -1];
...
{
#if (_DEBUG || _DEBUG_DECIMAL) && !_NODEBUG_DECIMAL
   if (dig > DFT_DEC_DIG || dig < 1 || dig < prec || prec < 0)
   {
            ...
            const _DecimalT<dig2, prec2> & val2)
{
...
#if __OS400_TGTVRM__ >= 720
   __padd63((char *)&RetResult, RetResult.DigitsOf(),
            RetResult.PrecisionOf() - maxp,
   ...
   memcpy (&RetResult, tempbuf + __MAX_BUF_SIZE - RetBufSize,
           RetBufSize);
#endif
...
GEN_1ST_DEC_TEMPLATE_FUNC_RET_FLT(+,double,        double)
GEN_1ST_DEC_TEMPLATE_FUNC_RET_FLT(+,float,         float)
GEN_1ST_DEC_TEMPLATE_FUNC_RET_FLT(+,long double,   long double)
...
GEN_2ND_DEC_TEMPLATE_FUNC_RET_FLT(+,double,        double)
GEN_2ND_DEC_TEMPLATE_FUNC_RET_FLT(+,float,         float)
GEN_2ND_DEC_TEMPLATE_FUNC_RET_FLT(+,long double,   long double)
...
#if __OS400_TGTVRM__ >= 720
   __psub63((char *)&RetResult, RetResult.DigitsOf(),
            RetResult.PrecisionOf() - maxp,
            (char *)&val1, dig1, maxp - prec1,
            (char *)&val2, dig2, maxp - prec2);
#else
...
#if __OS400_TGTVRM__ >= 720
   __pmpy63 ( (char *)&RetResult, RetResult.DigitsOf(),
              RetResult.PrecisionOf()-maxp,
              (char *)&val1, dig1,
              (char *)&val2, dig2);
#else
            ...
            const _DecimalT<dig2, prec2> & val2)
{
   ...
   __pdiv ( (char *)&RetResult, _DIV_BUFSZ(dig1,dig2), 0,
            (char *)&val1, dig1, adjq,
            (char *)&val2, dig2, 0);
...
operator / (const _ConvertDecimal & val1, const _ConvertDecimal & val2)
{
   ...
   __pdiv ( (char *)&RetResult, _DIV_BUFSZ(val1.DigitsOf(),val2.DigitsOf()), 0,
...
operator / (const _ConvertDecimal & val1, const _DecimalT<dig2,prec2> & val2)
{
   ...
   __pdiv ( (char *)&RetResult, _DIV_BUFSZ(val1.DigitsOf(),dig2), 0,
            (char *)&val1, val1.DigitsOf(), adjq,
            (char *)&val2, dig2, 0);
...
operator / (const _DecimalT<dig1,prec1> & val1, const _ConvertDecimal & val2)
{
   ...
   __pdiv ( (char *)&RetResult, _DIV_BUFSZ(dig1,val2.DigitsOf()), 0,
            (char *)&val1, dig1, adjq,
            (char *)&val2, val2.DigitsOf(), 0);
...
{
   char output[DFT_DEC_DIG+10];
   char format[15];
   sprintf (format, "%%D(%d,%d)", val.DigitsOf(), val.PrecisionOf());
